...
## Iterator Pattern 迭代器模式  
  用于顺序访问集合中的元素，而不需要暴露集合中的内部实现细节。该模式的目的是提供一个统一的方式来遍历各种集合结构。  
  在unity中用IEnumerator和IEnumerable来实现该模式，实现迭代的效果。  
### Main Structure 主要的结构  
  ·Normal class 普通的类，这个可以是敌人、场景管理、资源管理等等。  
  ·ConcreteAggregate 具体的聚合类。该类中主要有这几种方法：向普通类的列表中增加该类、获取迭代器，使用迭代器逐个遍历这个列表中所包含的类、还有一个非泛型版本的，为了兼容性而保留。  
### Main scene to use this pattern  
  ·集合的遍历  
  ·协程  
  ·游戏场景中遍历敌人，更新其状态  
  ·其实对于动画也是可以使用迭代器模式，按照先后的顺序来进行遍历，然后调整播放动画  
